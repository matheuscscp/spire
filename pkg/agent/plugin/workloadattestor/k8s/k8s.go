...
func (p *Plugin) buildConfig(coreConfig catalog.CoreConfig, hclText string, status *pluginconf.Status) *k8sConfig {
	...
	apiServerCacheConfig := buildAPIServerCacheConfig(newConfig.APIServerCache, status)
...
func buildAPIServerCacheConfig(hclConfig *k8sAPIServerCacheHCLConfig, status *pluginconf.Status) k8sAPIServerCacheConfig {
	if hclConfig == nil {
...
// plural+group → GVK and determine namespace scoping at runtime. The
// client, mapper, and cache share an HTTP client to avoid duplicate
// connection pools and duplicate discovery caches. Guarded by a dedicated
// mutex so the apiserver
// discovery handshake on first use does not block readers of unrelated
// plugin state.
func (p *Plugin) getOrCreateKubeClient(ctx context.Context) (client.Client, error) {
	p.kubeMu.RLock()
	if p.kubeClient != nil {
		c := p.kubeClient
		p.kubeMu.RUnlock()
	...
	// This cache will hold only corev1.Pod and metav1.PartialObjectMetadata,
	// which is quite fine in terms of memory consumption. We use PartialObjectMetadata
	// specifically to avoid caching large objects, and some clusters have thousands of
	// them (e.g. large Helm release Secrets).
	// Pod is a special exception as we emit specific selectors for pods that
	// require non-metadata fields, e.g. spec.serviceAccountName.
	...
	clientOptions.Cache = &client.CacheOptions{
		...
		// We want caching disabled for everything except corev1.Pod and
		// metav1.PartialObjectMetadata, but the client.CacheOptions API
		// doesn't support an allow-list, only a deny-list.
	}
	c, err := client.New(restConfig, clientOptions)
	if err != nil {
		cacheCancel()
