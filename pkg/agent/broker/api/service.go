...
type Config struct {
	...
	// URLs each authenticated caller (broker SPIFFE ID) may use. A caller
	// missing from the map has no restriction (the broker config builder
	// omits brokers that opted into "*"). The set values are full protobuf
	// type URLs (e.g.
	// `type.googleapis.com/spiffe.broker.KubernetesObjectReference`).
	AllowedReferenceTypesByCaller map[spiffeid.ID]map[string]struct{}

	// AllowedReferenceTypesOverTCP is the global allowlist of WorkloadReference
	// type URLs permitted on TCP connections. Empty denies every TCP request
	// (fail-closed default). No wildcard is honoured — operators must
	// enumerate the type URLs explicitly. UDS connections are unaffected.
	AllowedReferenceTypesOverTCP map[string]struct{}
}
...
func New(config Config) *Service {
	return &Service{
		...
		allowedReferenceTypesOverTCP:  config.AllowedReferenceTypesOverTCP,
	}
}
...
// authorizeReferenceType applies two gates: (1) the per-broker allowlist,
// (2) the global TCP allowlist when the connection is a TCP peer. Either
// gate failing yields PermissionDenied. A caller missing from the per-broker
// map has no per-broker restriction (the broker opted into "*"). The TCP
// gate is explicit-or-nothing — it only admits requests whose reference
// type URL is in the configured set, defaulting to deny-all when the set
// is empty. No wildcard is honoured for the TCP gate by design.
func (s *Service) authorizeReferenceType(ctx context.Context, caller spiffeid.ID, ref *anypb.Any) error {
	...
	allowed, ok := s.allowedReferenceTypesByCaller[caller]
	if ok {
		if _, ok := allowed[ref.GetTypeUrl()]; !ok {
			return status.Errorf(codes.PermissionDenied, "broker %q is not allowed to use reference type %q", caller, ref.GetTypeUrl())
		}
	}
	if isTCPCaller(ctx) {
		if _, ok := s.allowedReferenceTypesOverTCP[ref.GetTypeUrl()]; !ok {
			return status.Errorf(codes.PermissionDenied, "reference type %q is not allowed over TCP", ref.GetTypeUrl())
		}
	}
	return nil
}
