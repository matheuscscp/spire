...
type Config struct {
	...
	// type URLs permitted on the TCP listener. Empty denies every TCP
	// request with PermissionDenied. Wildcards are not honoured for this
	// gate — entries are verbatim type URLs. UDS connections are not
	// gated by this list.
	AllowedReferenceTypesOverTCP []string
...
func New(c *Config) (*Endpoints, error) {
	...
	case c.Attestor == nil:
		return nil, errors.New("attestor is required")
	case c.SVIDSource == nil:
		return nil, errors.New("SVID source is required")
...
func (e *Endpoints) ListenAndServe(ctx context.Context) error {
	unaryInterceptor, streamInterceptor := middleware.Interceptors(
		middleware.Chain(
			...
			middleware.Preprocess(restrictReflectionToUDS),
...
func (e *Endpoints) registerBrokerAPI(server *grpc.Server) {
	service := brokerapi.New(brokerapi.Config{
		...
		AllowedReferenceTypesOverTCP:  buildAllowedReferenceTypesOverTCPSet(e.c.AllowedReferenceTypesOverTCP),
	})

	brokerapi.RegisterService(server, service)
}

// buildAllowedReferenceTypesOverTCPSet materializes the global TCP allowlist
// into a set for O(1) membership checks. An empty input set means every
// TCP request will be rejected at the api service.
func buildAllowedReferenceTypesOverTCPSet(types []string) map[string]struct{} {
...
func restrictReflectionToUDS(ctx context.Context, fullMethod string, _ any) (context.Context, error) {
	...
	if p.Addr.Network() != "unix" {
		return nil, status.Error(codes.PermissionDenied, "server reflection is only available over Unix sockets")
	}
	return ctx, nil
}

func isReflectionMethod(fullMethod string) bool {
	return strings.HasPrefix(fullMethod, "/"+middleware.ServerReflectionServiceName+"/") ||
		strings.HasPrefix(fullMethod, "/"+middleware.ServerReflectionV1AlphaServiceName+"/")
}
